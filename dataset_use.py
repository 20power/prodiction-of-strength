...
class MyDataset(Dataset):
    def __init__(self, csv_path,max_len = None,is_twist = False):
        ...
        self.one_hot_map = pd.read_pickle('./one_hot_map_without.pkl')
        ...
        self.single_twist = ['纺纱股数','实测单纱捻度']
        self.ply_twist = ['纺纱股数','实测股线捻度']
        ...
        # HVI指标（基础）与 AFIS 指标
        self.base_hvi_cols = ['MIC','MAT%','LEN(INCH)','STR(CN/TEX)']
    ...
    def __getitem__(self, idx):
        ...
        sp_ps = torch.from_numpy(np.array(np.concatenate((sp,ps),axis=0),dtype=np.float32)).float()
...
if __name__ == "__main__":
    dataset = MyDataset('./single_test_data_600.csv',max_len=3)
